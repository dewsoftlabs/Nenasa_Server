...
const permissionByroleid = (req, res) => {
  ...
  UserRoleModel.getUserById(userid, (error, results) => {
    ...
    UserRoleModel.getUserPermission(
      userRoleIds,
      userid,
  ...
  function updateExistingUserrole(userRole, userRoleId) {
    UserRoleModel.updateUserRole(userRole, userRoleId, (error, results) => {
      ...
      if (results.affectedRows === 0) {
        res
          .status(404)
          .send({ error: "user role not found or no changes made" });
        return;
      }
...
const deleteRoles = (req, res) => {
  ...
  for (const userRoleId of userRoleIds) {
    UserRoleModel.getUserRoleById(userRoleId, (error, results) => {
      if (error) {
        console.error(
          `Error fetching user role with ID ${userRoleId}: ${error}`
        );
        failCount++;
        ...
        UserRoleModel.deleteUserRole(
          userRoleId,
          1,
          (deleteError, deleteResult) => {
            if (deleteError) {
              console.error(
                `Error deleting user role with ID ${userRoleId}: ${deleteError}`
              );
              failCount++;
            } else {
              successCount++;
              console.log(
                `User role with ID ${userRoleId} deleted successfully`
              );
            }
...
const permanentDeleteUserRole = (req, res) => {
  ...
  UserRoleModel.permanentDeleteUserRole(userRoleId, (error, results) => {
    if (error) {
      res
        .status(500)
        .send({ error: "Error deleting user role from the database" });
      return;
    }

    res
      .status(200)
      .send({ message: "UserRole permanently deleted successfully" });
  });
};
