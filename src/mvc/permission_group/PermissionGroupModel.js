...
const AssignPermissionModel = {
  ...
  getAssignPermissionByCode(assignPermissioncode, callback) {
    connection.query(
      "SELECT * FROM assign_permission WHERE permission_code = ? AND is_delete = 0",
      [assignPermissioncode],
      callback
    );
  },
  ...
  updateAssignPermissionStatus(assignPermissionId, status, callback) {
    const query =
      "UPDATE assign_permission SET status = ? WHERE assignpermissionid = ?";
  ...
  deleteAssignPermissions(assignPermissionIds, callback) {
    ...
    for (const assignPermissionId of assignPermissionIds) {
      AssignPermissionModel.getAssignPermissionById(
        assignPermissionId,
        (error, results) => {
          if (error || results.length === 0) {
            ...
            AssignPermissionModel.deleteAssignPermission(
              assignPermissionId,
              1,
