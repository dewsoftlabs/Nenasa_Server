...
const InstallementModel = {
  getInstallementById(installement_id, callback) {
    connection.query('SELECT * FROM installement WHERE installement_id = ? AND is_delete = 0', [installement_id], callback);
  },
  ...
  updateInstallement(installement, installement_id, callback) {
    const { collection_id, installment_desc, installement_amount, paid_amount, installement_status, installement_paidDate, userid } = installement;
    const query = 'UPDATE installement SET collection_id = ?, installment_desc = ?, installement_amount = ?, paid_amount = ?, installement_status = ?, installement_paidDate = ?, userid = ? WHERE installement_id = ?';
    const values = [collection_id, installment_desc, installement_amount, paid_amount, installement_status, installement_paidDate, userid, installement_id];

    connection.query(query, values, callback);
  },

  deleteInstallement(installement_id, value, callback) {
    const query = 'UPDATE installement SET is_delete = ? WHERE installement_id = ?';
