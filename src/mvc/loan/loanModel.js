...
const loanModel = {
  getloanById(loan_id, callback) {
    connection.query(
      "SELECT * FROM loan WHERE loan_id = ? AND is_delete = 0",
      [loan_id],
      callback
    );
  },
  ...
  getloanBybusinessName(business_name, callback) {
    connection.query(
      "SELECT * FROM loan WHERE business_name = ? AND status != 4 AND is_delete = 0",
      [business_name],
      callback
    );
  },
  ...
  addLoan(customer_id, deposit_acc_no, guarantor_id, loan, callback) {
    ...
    const values = [
      ...
      deposit_amount,
      total_payamount,
      activeValues, // Assuming status is TINYINT(1)
